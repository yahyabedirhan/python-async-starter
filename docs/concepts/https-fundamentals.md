...
solved and where each piece (Caddy, sslip.io, Let's Encrypt) fits. General
knowledge / background explanation — not tied to one fetched source, unlike
`docs/concepts/caddy.md` and `docs/concepts/sslip-io.md`, which cite the
specific tool docs.
...
- **SSL vs. TLS**: SSL (Secure Sockets Layer) was the original protocol
  for encrypting a connection. It's old and has known security flaws — it
  was formally replaced by **TLS** (Transport Layer Security) years ago.
...
- **DV / OV / EV** — three levels of how much a CA verifies before
...
protocol numbers) to long-established services — 22 for SSH, 25 for email
(SMTP), 80/443 for the web, and so on. On Linux, there's a practical
...
- **ZeroSSL** — Caddy's own automatic fallback if Let's Encrypt fails;
  works the same way (free, automated, DV).
- Paid CAs (DigiCert, Sectigo, GlobalSign) — used when an organization
...
- **Nginx + Certbot** — the older, more manual combination: Nginx as the
  ...
  both natively.
- **Traefik** — a reverse proxy comparable to Caddy, also with automatic
  HTTPS built in via ACME. Especially popular in Docker/Kubernetes setups
  where services come and go dynamically.
- **A CDN/proxy in front of the origin** (e.g. Cloudflare's proxy mode) —
