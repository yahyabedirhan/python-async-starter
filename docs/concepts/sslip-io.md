...
addresses." Its own README gives the example: `127-0-0-1.sslip.io`
...
text and returns them as the IP address, with no registration, no config,
and no account. Any IP address automatically has a working sslip.io
hostname, because the "database" is just the hostname's own text.
